...
const { SECTORS, ETFS, REGIME_BENCHMARK, findByQuery, buildSymbolIndex } =
  await import("./src/universe.js");
const { fetchCandles, isMock } = await import("./src/toss.js");
const { rawFactors, scoreUniverse, determineRegime, signalFor } =
  await import("./src/amqs.js");
...
async function loadUniverseFactors() {
  ...
  for (const s of SECTORS)
    for (const st of s.stocks)
      items.push({ ...st, sector: s.name, sectorKey: s.key, type: "stock" });
  for (const e of ETFS)
    items.push({ ...e, sector: "ETF", sectorKey: "etf", type: "etf" });
...
async function buildDashboard() {
  if (dashboardCache.data && Date.now() - dashboardCache.at < DASH_TTL)
    return dashboardCache.data;

  const [benchCandles, items] = await Promise.all([
    getCandles(REGIME_BENCHMARK.code),
    loadUniverseFactors(),
  ]);
  ...
  for (const arr of byKey.values())
    arr.sort((a, b) => (b.momentum_score ?? 0) - (a.momentum_score ?? 0));
...
app.get("/api/search", async (req, res) => {
  ...
  if (!q)
    return res.status(400).json({
      error: "검색어(q)가 필요합니다. 종목명 또는 6자리 코드를 입력하세요.",
    });
  ...
  try {
    ...
    if (!meta && /^\d{6}$/.test(q))
      meta = { code: q, name: q, sector: "검색", type: "unknown" };
    if (!meta)
      return res.status(404).json({
        error: `'${q}' 종목을 찾지 못했습니다. 6자리 코드로 시도해 보세요.`,
      });

    // 2) 횡단면 z-score를 위해 universe factors + 검색 종목을 함께 점수화
    const [benchCandles, universe] = await Promise.all([
      getCandles(REGIME_BENCHMARK.code),
      loadUniverseFactors(),
    ]);
    const regime = determineRegime(benchCandles);
...
app.listen(PORT, () => {
  console.log(
    `AMQS 대시보드: http://localhost:${PORT}  (mode: ${IS_MOCK ? "MOCK" : "TOSS LIVE"})`,
  );
});
