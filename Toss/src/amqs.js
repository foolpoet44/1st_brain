...
function zscores(values) {
  ...
  const sd =
    ...
    1;
  return values.map((v) =>
    v != null && Number.isFinite(v) ? (v - mean) / sd : 0,
  );
}
...
export function scoreUniverse(items) {
  ...
  const composite = items.map(
    (_, i) => 0.5 * z12[i] + 0.3 * z6[i] + 0.15 * z3[i] + 0.05 * zv[i],
  );
...
export function determineRegime(benchmarkCandles) {
  ...
  const ret5d =
    series.length >= 6 ? (now / series[series.length - 6] - 1) * 100 : 0;
...
export function signalFor(scored, regime) {
  ...
  const drawdownFromHigh =
    f.trailingHigh && price ? (price / f.trailingHigh - 1) * 100 : 0;
  const reasons = [];
