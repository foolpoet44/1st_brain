...
const baseUrl = () =>
  process.env.TOSS_BASE_URL || "https://openapi.tossinvest.com";
...
async function getAccessToken() {
  ...
  if (!res.ok)
    throw new Error(`Toss OAuth 실패: ${res.status} ${await res.text()}`);
...
async function authedGet(path, params) {
  ...
  if (!res.ok)
    throw new Error(`Toss API ${path} 실패: ${res.status} ${await res.text()}`);
  return res.json();
}
...
export async function fetchPrices(codes) {
  ...
  for (let i = 0; i < codes.length; i += 200) {
    ...
    const json = await authedGet("/api/v1/prices", {
      symbols: chunk.join(","),
    });
...
export async function fetchCandles(code, days = 260) {
  ...
  // 과거 → 현재 정렬 + 중복 제거
  const uniq = dedupeByTime(collected).sort((a, b) =>
...
export function mockCandles(code, days = 260) {
  ...
  for (let i = 0; i < days; i++) {
    const t = i / days;
    const localDrift =
      t > flipAt ? dailyDrift * (rand() < 0.5 ? -1.4 : 1.2) : dailyDrift;
    // box-muller
