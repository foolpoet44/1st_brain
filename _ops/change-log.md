...
2. **`workflow_dispatch` 호출 한 줄 추가(어제 3-(a), 범위 축소)** — `pages.yml` 에 트리거는 **이미 있다.** `deploy-visual.yml` · `weekly-digest.yml` 말미에서 Pages 워크플로를 dispatch 하도록 한 스텝만 붙이면 된다. PAT 확대(b 안)는 여전히 불필요
3. **[[vibe-coding]] 재소화 — 미이행으로 명시 이월(108 일째)** — 취소가 아니라 미이행이다. 「'왜'보다 '무엇을'」과 `CLAUDE.md` 의 「'왜'를 반드시 포함」을 같은 문단에서 마주치게 하고 `status: seed → growing`, 오타 「맡려서」 정정. 다만 1·2 번 다음으로 순위 조정
4. **76% → 98% 정정(닷새째 이월)** — `2026-07-22-autonomous-hiring-paradox.md` `346` 행. 정답은 같은 파일 `269` 행
5. **8.5% 검증 또는 철회(닷새째 이월)** — `2026-08-10-capital-flow-market-neutral.md` `44`·`47` 행
6. **브리핑 파이프라인 침묵 원인 확인(신규)** — W32 최활발 영역(48 건)이 08-13 이후 0 건이다. 스케줄 종료인지 실행 실패인지 확인해야 이틀째 침묵이 정지인지 설계인지 판정된다
7. **검증자 분리(77 일째)** — 오늘도 같은 서명이다. 어제의 지시를 오늘의 내가 어겼고, 그것을 채점한 것도 나다. 이 항목이 미해결인 한 나머지 전부가 미해결일 수 있다
