...
- [x] 에이전트가 제시한 해결책이 기존 시스템의 다른 스킬과 충돌하지 않는가?
- [x] "성공했습니다"라는 보고의 물리적 증거(파일 생성, API 응답 로그)가 확인되었는가?
- [x] 예외 상황(Error Handling)에 대한 고려가 생략되지 않았는가?

## 2. 인지적 편향 제거

- [x] **Anti-Optimism Check**: 가장 낙관적인 시나리오가 아닌, 최악의 시나리오에서도 시스템이 내구성을 갖는가?
