...
- **중복 대조**: 16 개 기존 문서 검색, 최다 일치 10 개 (2026-08-10-capital-flow-market-neutral.md)
- **편입**: MERGE 1 건 (기존 문서 Timeline 에 append)
- **신규**: 0 건 (브리핑 제안 불-follow — "브리핑은 자기가 무엇과 중복되는지 모른다")
- **중복 종결**: 0 건

**판정 근거**:
- 29% EMN, $5.22T, 65% 기관투자자는 기존 문서와 중복
- **새로운 신호**: 금리 인상 확률 56.5% (PCE 3.7%), 한국은행 3.00% 인상 — 기존 문서에 없음
- Human Gate 3 종 추출 (금리 인상 시나리오 공개, 내외금리차 감시, 반도체 호황 기망 감시)

---

### 2026-09-01 — INGEST 실행

- **수집**: 1 개 파일 (BRIEFING_MONEY-FLOW_2026-09-01.md)
- **중복 대조**: 16 개 기존 문서 검색, 최다 일치 10 개 (2026-08-10-capital-flow-market-neutral.md)
- **편입**: MERGE 1 건 (기존 문서 Timeline 에 append)
- **신규**: 0 건 (브리핑 제안 불-follow — "브리핑은 자기가 무엇과 중복되는지 모른다")
- **중복 종결**: 0 건
...
**판정 근거**:
- 29% EMN, $5.22 조, 60/20/20 포트폴리오 등 핵심 통계가 기존 문서와 중복
- Human Gate 4 종 추출 (자본 중립성 심의회, 금리 민감도 공개, 환율 임계치 감시, 인플레이션 헤지 예산)

## 2026-08-25 — INGEST 수행 (아침 브리핑 2 건)
...
1. **BRIEFING_MONEY-FLOW_2026-08-23.md** → `2026-08-10-capital-flow-market-neutral.md`
   - 핵심 통계: $5.22 조 (HFR AUM), 29% (Equity Market Neutral), Fed 3.50-3.75% (3 명 반대), 환율 1,414 원, 해외투자 $1,403 억 (109% 증가)
   - 판정: 5 개 통계 모두 기존 문서와 중복 → MERGE

2. **BRIEFING_IO-PSYCH_2026-08-23.md** → `2026-07-22-autonomous-hiring-paradox.md`
   - 핵심 통계: Stanford HAI (4M 지원, 150+ 고용주, Black 26% 편향, Asian 15% 편향), 의사결정 피로 (35,000 건/일, 10 가지 원인), AI 노출 작업 역설 (창의성↔AI 노출 정비례)
   - 판정: Stanford HAI 통계가 기존 문서에 이미 편입되어 있음 → MERGE

### 판정 근거

- 통계 매칭 기준: 2 개 이상 일치 시 MERGE
- MONEY-FLOW: $5.22 조, 29% EMN, Fed 3.5-3.75%, 환율 1,414 원 — 모두 2026-08-10 문서에 존재
- IO-PSYCH: Stanford HAI 26% 편향 통계가 2026-07-22 문서에 이미 편입되어 있음 (2026-08-24 브리핑에서 재확인)
- **핵심 원칙**: "브리핑은 자기가 무엇과 중복되는지 모른다" — 브리핑이 제안한 신규 노드를 생성하지 않고, 기존 문서의 Timeline 에 병합

### Human Gate 추출 (IO-PSYCH 병합에서)

1. **알고리즘 공정성 감사 위원회 (DEI)** — 분기별 인간 심사 (벤더 다양성 영향 평가)
2. **의사결정 아키텍처 심의회** — 오후 2 시 이후 최종 거부 금지 (AI 기반 채용 거부는 14:00 이후 인간 관리자 1:1 면담 필수)
3. **에이전트 조직 설계 심의회** — 인간 모방 구조 금지 ('AI recruiter' 직함 금지)
