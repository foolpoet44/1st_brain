---
title: PPT를 HTML로 변환하는 데이터 전달 방식 평가
date: 2026-01-30
time: 05:51
uuid: 0d093a12-c224-4beb-bdbe-6d638aed9380
type: conversation
tags: [archive, conversation]
---

# PPT를 HTML로 변환하는 데이터 전달 방식 평가
...
첨부 문서를 레이아웃 - 스타일 그대로 Html(웹문서)로 전환하는 프로젝트를 진행하려고 합니다.
아래와 같은 방식으로 ppt 정보를 모델에 전달하려고 하는데, 적절성(최적화 관점)에서 평가해 주세요.
1️⃣ PPT → 구조·색상·텍스트 를 python‑pptx 로 XML/객체 로읽는다 → shape.left/top/width/height, slide.background.fill.fore_color.rgb, 테마 색 (a:clrScheme). 2️⃣ JSON 으로 정규화 → 위치·크기·색·텍스트·이미지를 모두 포함.

## Claude (Message 2)

# PPT→HTML 전환 프로젝트: 정보 전달 방식 평가

## 🎯 제안하신 방식의 핵심

```
PPT → python-pptx로 파싱 → JSON 정규화 → AI 모델에 전달 → HTML 생성
```
...
- JSON은 LLM이 가장 잘 이해하는 구조화된 데이터
- 규칙 기반 변환보다 의도 파악이 가능
- 레이아웃 재현을 위한 충분한 정보 제공

**3. 재현 가능성**

- 절대 좌표를 통한 pixel-perfect 재현
- 색상 정보 보존
- 계층 구조 유지
...
현재 방식은 "물리적 속성"에 집중되어 있습니다. HTML 변환의 본질을 고려하면:
...
- HTML은 시맨틱 구조가 핵심 (`<h1>`, `<section>`, `<article>`)
- AI가 "제목인가, 본문인가, 캡션인가"를 판단할 추가 정보 필요
- 반응형 디자인 시 의미 기반 재배치 가능
...
- PPT의 "이미지+텍스트 그룹"을 HTML `<figure>` + `<figcaption>`으로
- 목록 아이템들을 `<ul>` + `<li>`로 자동 인식
- 카드 레이아웃 감지
...
### PPT → HTML 변환의 본질적 딜레마

**PPT의 본질**: 고정된 캔버스 (1920×1080px)

- 절대 좌표 기반
- "이 위치에 이것을 그린다"

**HTML의 본질**: 유동적 문서 (responsive)
...
<header>
  <h1>제조 AX 아카데미</h1>
  <p class="subtitle">CDP 전문가 양성 과정</p>
</header>
```

- ✅ 장점: 반응형, SEO, 접근성
- ❌ 단점: 원본 레이아웃과 차이 발생 가능
...
```python
slide["groups"] = detect_visual_groups(shapes)
```
...
- 원본 PPT와 HTML을 나란히 비교
- 모바일 뷰 테스트
- Lighthouse 점수 확인 (접근성, SEO)
...
> python-pptx로 물리적 속성을 추출하는 것은 필수적이고 올바른 출발점입니다.  
> 하지만 그것만으로는 부족합니다.  
> "의미론적 레이어"를 추가해야 AI가 진정으로 "이해"하고 "좋은 HTML"을 생성할 수 있습니다.
...
제안된 방법으로 ppt파일을 파싱하는 에이전트를 설계할 수 있도록 프롬프트 문서를 생성해줘:
안티그래비티의 gemini3 모델을 사용할 예정

## Claude (Message 4)

# PPT→HTML 변환 파싱 에이전트 프롬프트 문서

## 🎯 에이전트 정의

당신은 **PPT Semantic Parser Agent**입니다. PowerPoint 파일을 분석하여 물리적 속성뿐만 아니라 의미론적 구조까지 파악하는 전문가입니다.

---

## 📋 Core Mission

PowerPoint 슬라이드를 다음 3개 레이어로 분석하여 JSON을 생성하세요:

1. **Physical Layer** (물리적 속성): 위치, 크기, 색상, 폰트
2. **Semantic Layer** (의미론적 속성): 역할, 계층, 콘텐츠 타입
3. **Relational Layer** (관계적 속성): 그룹, 레이아웃 패턴, 시각적 연결
...
| Physical Hints         | Semantic Role | HTML Suggestion             | Level |
| ---------------------- | ------------- | --------------------------- | ----- |
| 폰트 44pt+, 상단 중앙  | `heading`     | `<h1>`                      | 1     |
| 폰트 32pt+, 상단       | `heading`     | `<h2>`                      | 2     |
| 폰트 24pt+, 본문 영역  | `heading`     | `<h3>`                      | 3     |
| 폰트 18pt, 긴 텍스트   | `paragraph`   | `<p>`                       | -     |
| 폰트 14pt, 짧은 텍스트 | `label`       | `<span>`                    | -     |
| 테마 accent 색상       | `emphasis`    | `<strong>` or `<em>`        | -     |
| 리스트 정렬 패턴       | `list_item`   | `<li>`                      | -     |
| 하단, 작은 폰트        | `caption`     | `<figcaption>` or `<small>` | -     |
...
| Intent           | 설명                   | HTML 전환 시 활용               |
| ---------------- | ---------------------- | ------------------------------- |
| `hero_title`     | 슬라이드 주제/제목     | `<header>` + `<h1>`             |
| `subtitle`       | 부제목                 | `<p class="subtitle">`          |
| `section_header` | 섹션 구분자            | `<h2>` + `<hr>`                 |
| `body_content`   | 본문 내용              | `<article>` + `<p>`             |
| `callout`        | 강조 박스              | `<aside class="callout">`       |
| `cta`            | 행동 유도              | `<button>` or `<a class="cta">` |
| `metadata`       | 메타 정보 (날짜, 저자) | `<footer>` + `<small>`          |
| `decoration`     | 장식적 요소            | `aria-hidden="true"`            |
...
| Pattern              | 설명              | HTML 변환                    |
| -------------------- | ----------------- | ---------------------------- |
| `hero_centered`      | 상단 중앙 타이틀  | `<header>` + centered layout |
| `two_column`         | 2단 레이아웃      | CSS Grid 2-column            |
| `three_column_cards` | 3단 카드          | CSS Grid 3-column            |
| `sidebar_content`    | 사이드바 + 본문   | Flexbox sidebar              |
| `full_bleed_image`   | 전체 이미지 배경  | Background image + overlay   |
| `timeline`           | 타임라인 레이아웃 | Vertical stepper             |

---

## 🎨 Visual Weight & Attention Score
...
```json
{
  "metadata": {
    ...
    "theme": {
      ...
      "fonts": {
        "heading": {"latin": "string", "east_asian": "string"},
        "body": {"latin": "string", "east_asian": "string"}
      }
    }
  },
  "slides": [
    {
      ...
      "elements": [
        {
          ...
          "geometry": {
            ...
            "relative": {"h_align": "center|left|right", "v_align": "top|middle|bottom"}
          },
          "style": {
            "font": {"family": "", "size": 0, "weight": "", "color": ""},
            "fill": {"type": "", "color": ""},
            "effects": {"shadow": true|false}
          },
          "semantic": {
            "role": "heading|paragraph|list_item|label",
            "level": 1|2|3|null,
            "html_tag_suggestion": "h1|h2|p|li|span"
          },
          "design": {
            "intent": "hero_title|subtitle|body_content|callout|metadata",
      ...
      "groups": [
        {
          "id": "group_X",
          "type": "vertical_stack|card_grid|figure_with_caption",
          "members": ["elem_X_Y", "elem_X_Z"],
          "html_structure_suggestion": {
            "container": "div|section|figure",
            "layout_method": "flex|grid"
          }
        }
      ],
      "layout_analysis": {
        "pattern": "hero_centered|two_column|card_grid",
        "grid": {"detected": true|false, "columns": 0}
      }
    }
  ]
}
...
### 1. Gemini Flash 3.0 특성 활용

- **긴 컨텍스트 처리 능력**: 전체 PPT를 한 번에 전달 가능
- **구조화된 출력**: JSON 스키마를 명확히 제시
- **멀티모달 이해**: 이미지도 함께 분석 가능 (향후 확장)
...
- **Few-shot Example**: 완전한 JSON 예시 1개 제공
- **Chain-of-Thought**: "먼저 물리적 속성, 그 다음 의미 추론" 순서 강조
- **Validation**: "JSON이 valid한지 확인하세요" 명시
...
### Phase 3: HTML 생성 (다음 단계)

- Gemini에게 JSON + HTML 변환 프롬프트 전달
- 시맨틱 HTML5 생성
- CSS로 레이아웃 재현
- 반응형 디자인 적용
...
이제 실제 PPT 파일로 테스트해볼까요? 🎨

---

_Archived from: `0d093a12-c224-4beb-bdbe-6d638aed9380.json`_
