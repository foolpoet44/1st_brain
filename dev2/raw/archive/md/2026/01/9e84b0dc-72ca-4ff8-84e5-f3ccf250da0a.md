---
title: ESCO 기반 physical AI 역량 데이터베이스 구축
date: 2026-01-28
time: 09:17
uuid: 9e84b0dc-72ca-4ff8-84e5-f3ccf250da0a
type: conversation
tags: [archive, conversation]
---

# ESCO 기반 physical AI 역량 데이터베이스 구축
...
ESCO기반의 physical AI 역량 DB를 생성할거야
최적 방안을 제안해

## Claude (Message 2)

# Physical AI 역량 DB 구축: ESCO 기반 설계 전략

Physical AI 역량 데이터베이스를 ESCO 프레임워크 기반으로 구축하는 것은 매우 흥미로운 도전입니다. 이는 단순히 기술 스택을 나열하는 것이 아니라, **인간의 역량 체계를 Physical AI의 맥락으로 재해석**하는 작업이기 때문입니다.

## 🎯 핵심 설계 철학

Physical AI는 디지털 세계를 넘어 물리적 세계와 상호작용하는 AI입니다. 로봇공학, 센서 기술, 액추에이터 제어, 실시간 의사결정이 결합된 영역이죠. ESCO의 역량 체계를 이 맥락에 적용하려면:

1. **Skills (기술)**: 구체적이고 측정 가능한 능력
2. **Knowledge (지식)**: 이론적 배경과 원리 이해
3. **Competences (역량)**: 복합적인 문제 해결 능력

이 세 축을 Physical AI 도메인에 맞게 재구성해야 합니다.
...
### Option A: Lightweight SQLite 기반

**장점**: 빠른 프로토타이핑, 로컬 작업 용이
**적합**: MVP, 개인 프로젝트, 1만 개 이하 스킬
...
### Option B: PostgreSQL + pgvector 기반

**장점**: 시맨틱 검색, 확장성, 동시 접속
**적합**: 프로덕션, 팀 협업, 10만 개 이상 스킬
...
1. **빠른 시작**: SQLite로 스키마 검증 및 초기 데이터 구축
2. **점진적 확장**: PostgreSQL로 이관하며 벡터 검색 추가
3. **유연성**: Neo4j는 네트워크 분석이 필요할 때만 도입
...
- 로봇공학 커리큘럼과 매핑 검증
- Physical AI 논문에서 언급된 스킬과 비교
- 기업 JD에서 요구하는 스킬과 gap 분석

## 📋 데이터 소싱 전략

### Primary Sources

1. **ESCO API**: 공식 스킬 온톨로지
2. **O\*NET**: 미국 직업 정보 (robotics engineer 등)
3. **IEEE RAS**: 로봇공학 표준 및 교육 과정

### Validation Sources

- arXiv 로봇공학 논문 키워드 분석
- 주요 Physical AI 기업 채용공고 (Boston Dynamics, Figure, Tesla Bot)
- 대학 로봇공학 석사 과정 커리큘럼
...
1. **목적**: 이 DB를 어떻게 활용할 건가요?
   - 교육 과정 설계?
   - 채용/평가 시스템?
   - 연구 목적의 스킬 분류?

2. **범위**: Physical AI의 경계를 어디까지 잡을까요?
   - 산업용 로봇만?
   - Humanoid robots 포함?
   - Soft robotics, Bio-inspired robots?

3. **사용자**: 누가 이 DB를 사용하나요?
   - HR 담당자?
   - 엔지니어?
   - 학생/교육자?

이 질문들에 대한 답변에 따라 설계를 더 구체화할 수 있습니다.
...
- ESCO 데이터 필터링 스크립트?
- 샘플 데이터 생성?
- 특정 도메인(예: manipulation) 상세 설계?

## Human (Message 3)

활용목적은 제한사항이 없고, 일단 산업용 로봇만, 사용자는 HR/엔지니어
원본 DB를 추출할 수 있도록 하는 claude code용 프롬프트 문서로 만들어줘 :

## Claude (Message 4)

# Physical AI 역량 DB 구축: Claude Code 실행 프롬프트

## 🎯 프로젝트 개요

**목적**: ESCO 기반 산업용 로봇(Industrial Robotics) 역량 데이터베이스 구축  
**사용자**: HR 담당자 및 로봇 엔지니어  
**범위**: 산업용 로봇 도메인에 특화된 스킬/지식/역량 체계  
**출력**: SQLite 데이터베이스 + 검증 리포트 + 샘플 쿼리
...
### Phase 5: Physical AI 확장 속성 자동 추론
...
실행 완료 후 다음 파일들이 생성되어야 함:

- [ ] `physical_ai_skills.db` - 메인 데이터베이스 (5-10 MB)
- [ ] `README.md` - 종합 가이드 (5-10 페이지)
- [ ] `data_dictionary.json` - 데이터 사전
- [ ] `statistics_report.md` - 통계 리포트
- [ ] `sample_queries.sql` - 샘플 쿼리 모음
- [ ] `validation_queries.sql` - 검증 쿼리
- [ ] `validation_results.json` - 검증 결과
- [ ] `quick_start_guide.md` - 빠른 시작 가이드
- [ ] `hr_dashboard_queries.sql` - HR 대시보드 쿼리
- [ ] `filtering_log.txt` - 필터링 로그
...
이 문서를 Claude Code에 복사하여 붙여넣고, 각 Phase를 순차적으로 실행하도록 지시하세요:

```
위 프롬프트의 Phase 1부터 Phase 10까지 순차적으로 실행해줘.
각 Phase 완료 후 진행 상황을 알려주고, 오류 발생 시 상세히 보고해줘.
```
...
필요에 따라 다음 확장 기능을 요청할 수 있습니다:

1. **시각화 대시보드**: 스킬 네트워크 그래프, 학습 경로 플로우차트
2. **API 서버**: FastAPI 기반 REST API 구축
3. **Excel 내보내기**: HR 팀용 스킬 매트릭스 Excel 생성
4. **비교 분석**: 경쟁사 JD 크롤링 및 스킬 갭 분석
5. **온톨로지 시각화**: Graphviz 기반 스킬 계층 구조 다이어그램

---

_Archived from: `9e84b0dc-72ca-4ff8-84e5-f3ccf250da0a.json`_
