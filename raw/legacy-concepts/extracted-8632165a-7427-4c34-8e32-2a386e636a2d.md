---
type: Resource
---

﻿# Extracted Knowledge from Conv: 8632165a-7427-4c34-8e32-2a386e636a2d

**Date**: 2026-01-24T01:13:06.207255Z

### Extracted Code (text)

```text
brainstorm → plan → implement 워크플로우
- test-driven-development (TDD 강제)
- systematic-debugging (4단계 디버깅)
- verification-before-completion (완료 전 검증)
```

### Extracted Code (text)

```text
User → Router → Swarm → [[AGENTS.md|AGENTS]] → Memory → LLM Providers
        ↑                                        ↓
        └────────── Learning Loop ←──────────────┘
```

### Extracted Code (text)

```text
사용자: "plan this"
→ 자동으로 Planner agent 활성화 + 요구사항 인터뷰 시작

사용자: "don't stop until done"
→ Ralph loop 활성화 (완료 검증까지 자동 순환)
```

### Extracted Code (markdown)

```markdown
---
name: hr-ontology-architect
trigger: ["온톨로지", "역량 체계", "스킬 맵", "competency framework"]
purpose: HR 역량/스킬을 온톨로지로 자동 구조화
---

# HR Ontology Architect [[Understand-Anything[[Understand-Anything/understand-anything-plugin/skills/understand/SKILL.md|SKILL]]-anything-plugin/skills/understand-knowledge/SKILL.md|SKILL]]

## When to Activate

- 역량 체계 설계 요청 시
- ESCO API 데이터 분석 시
- 리더십 평가 프레임워크 구축 시
- "스킬 온톨로지" 키워드 감지 시

## Process

1. **Domain Analysis** (도메인 분석)
   - 입력 데이터 파악 (Excel, CSV, API response)
   - 핵심 개념(concept) 추출
   - 관계(relation) 식별

2. **Ontology Generation** (온톨로지 생성)
   - 클래스 계층 구조 생성
   - 속성(property) 정의
   - 제약조건(constraint) 명시

3. **Validation** (검증)
   - 순환 참조 체크
   - 일관성 검증
   - 사용자 피드백 반영

4. **Output Formats** (출력 형식)
   - Mermaid diagram (시각화)
   - JSON-LD (기계 가독)
   - Notion 데이터베이스 스키마
   - Excel taxonomy

## Example Usage

**Input:**
"ESCO API에서 가져온 디지털 스킬 데이터를 온톨로지로 변환해줘"

**Output:**
```

### Extracted Code (markdown)

```markdown
---
name: leadership-feedback-composer
trigger: ["피드백", "평가 리포트", "leadership report", "360도"]
purpose: 리더십 평가 데이터를 개인화된 피드백 문서로 자동 변환
---

# Leadership Feedback Composer Skill

## When to Activate

- 평가 데이터 Excel/CSV 업로드 시
- "피드백 리포트 생성" 요청 시
- "리더십 평가 결과" 키워드 감지 시

## Psychological Framework

**Feedback Sandwich 2.0**:

1. Context Setting (맥락 설정)
2. Strength Recognition (강점 인정)
3. Growth Opportunity (성장 기회)
4. Actionable Steps (구체적 행동)
5. Encouragement (격려)

## Process

1. **Data Ingestion**
   - CSV/Excel 파싱
   - 평가 항목 매핑
   - 통계 분석 (평균, 백분위)

2. **Narrative Generation**
   - 강점 자동 추출 (상위 20% 항목)
   - 개선 영역 식별 (하위 20% 항목)
   - 동료 비교 분석

3. **Document Assembly**
   - DOCX skill 활용
   - 개인화된 인사말
   - 데이터 시각화 (차트)
   - 액션플랜 템플릿

4. **Quality Check**
   - 톤 일관성 검증
   - 민감 표현 필터링
   - 개인정보 마스킹

## Template Structure

### Page 1: Executive Summary

- 전체 평가 등급
- 핵심 강점 3가지
- 개선 영역 2가지

### Page 2: Detailed Analysis

**역량별 분석**:

- 전략적 사고: 85/100 (상위 15%)
  - 강점: "장기 비전 수립 능력 탁월"
  - 개선: "단기 실행 계획 구체성 보완 필요"

### Page 3: 360도 피드백

- 상사 관점
- 동료 관점
- 부하 관점

### Page 4: Action Plan
```

### Extracted Code (markdown)

```markdown
---
name: second-brain-architect
trigger: ["세컨브레인", "Notion 구조", "지식 체계", "PKM"]
purpose: Notion 워크스페이스를 최적 구조로 설계 및 자동화
---

# Second Brain Architect Skill

## When to Activate

- Notion 데이터베이스 설계 요청
- "세컨브레인 구조" 키워드
- 지식 관리 시스템 구축 시

## Design Philosophy

**PARA + Ontology Hybrid**:

- **P**rojects (프로젝트): 진행 중인 작업
- **A**reas (영역): 지속적 책임 영역
- **R**esources (자료): 참고 자료
- **A**rchives (보관): 완료/비활성

* **Ontological Relations** (온톨로지 관계):

- isPartOf (부분)
- relatedTo (관련)
- derivedFrom (파생)
- prerequisiteFor (선행조건)

## Workflow

### 1. 현재 구조 분석
```

### Extracted Code (markdown)

```markdown
---
name: rapid-prototype-weaver
trigger: ["프로토타입", "MVP", "빠르게", "테스트", "vibe coding"]
purpose: 아이디어를 2시간 내 작동 가능한 프로토타입으로 전환
---

# Rapid Prototype Weaver Skill

## When to Activate

- "빠르게 만들어줘" 키워드
- MVP 요청
- 개념 증명(PoC) 필요 시
- Vibe coding 모드

## Philosophy: Minimum Viable Learning

목표는 완벽한 제품이 아니라 **최대한 빨리 배우기**

## 80/20 Rule Application

- 20% 시간으로 80% 기능 구현
- 나머지는 반복(iteration)으로 개선

## Workflow

### Phase 1: 5분 - Essence Extraction

**질문:**

1. 핵심 가치는? (한 문장)
2. 첫 사용자는? (페르소나)
3. 성공 지표는? (측정 가능한 것)

### Phase 2: 10분 - Stack Selection

**Vibe Coder를 위한 스택:**
```

### Extracted Code (text)

```text
### Phase 5: 30분 - Demo & Feedback
- 실제 데이터로 테스트
- 오류 수정
- 사용자(자신) 피드백

## Anti-Patterns (하지 말 것)
❌ 완벽한 UI 디자인에 시간 쓰기
❌ 엣지 케이스 모두 처리하기
❌ 확장성 미리 고민하기
❌ 문서화 먼저 하기

## Do-Patterns (할 것)
✅ 작동하는 것부터
✅ 하드코딩 OK (나중에 변수화)
✅ print() 디버깅 OK
✅ 복붙 코드 OK (나중에 리팩토링)

## CSP Use Cases

### Example 1: ESCO 스킬 매핑 도구
```

### Extracted Code (markdown)

```markdown
---
name: context-engineering-master
trigger: ["복잡한 프로젝트", "장기 작업", "context", "planning"]
purpose: Manus의 $2B 비밀 - 파일 기반 컨텍스트 관리
---

# Context Engineering Master Skill

## When to Activate

- 복잡한 멀티스텝 프로젝트
- 여러 세션에 걸친 작업
- "계획" 키워드
- 컨텍스트 윈도우가 차기 시작할 때

## Manus Philosophy

**"Markdown is my working memory on disk"**

## 3-File System

### 1. project_plan.md - 나침반
```

### Extracted Code (bash)

```bash
# 1. Context Engineering으로 시작
/planning-with-files:start

# 2. project_plan.md 확인
"이번 주 목표: 리더십 평가 자동화 MVP 완성"

# 3. 온톨로지 설계
"ESCO 디지털 스킬 데이터를 온톨로지로 변환해줘"
→ HR-Ontology-Architect 자동 활성화

# 4. 빠른 프로토타입
"평가 데이터 분석하는 streamlit 앱 만들어줘"
→ Rapid-Prototype-Weaver 활성화

# 5. 피드백 리포트 생성
"이 데이터로 10명의 리더십 피드백 문서 생성"
→ Leadership-Feedback-Composer 활성화

# 6. Notion에 정리
"오늘 작업 내용을 세컨브레인에 아카이빙"
→ Second-Brain-Architect 활성화

# 7. 세션 종료
→ progress.md 자동 업데이트
→ 다음 세션을 위한 컨텍스트 저장
```
